...
resource "google_cloud_run_service" "bluelion-backend" {
  ...
  template {
    spec {
      ...
      containers {
        image = "europe-west1-docker.pkg.dev/ceri-m1-ecommerce-2022/bluelion/backend:0.0.19"
        env {
...
resource "google_cloud_run_service" "bluelion-frontend" {
  ...
  template {
    spec {
      service_account_name = "terraform-bluelion@ceri-m1-ecommerce-2022.iam.gserviceaccount.com"
      containers {
        image = "europe-west1-docker.pkg.dev/ceri-m1-ecommerce-2022/bluelion/frontend:0.0.19"
        env {
          name = "BACKEND_URL"
